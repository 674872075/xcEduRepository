...
public class MediaUploadService {
    ...
    private ResponseResult sendProcessVideoMsg(String fileId) {
        ...
        //向MQ发送视频处理消息
        try {
            rabbitTemplate.convertAndSend(RabbitMQConfig.EX_MEDIA_PROCESSTASK, routingkey_media_video, jsonStr);
        }catch (Exception e){
            logger.error("向MQ发送视频处理消息时出现错误:{}",e.toString());
            return ResponseResult.FAIL();
        }
        logger.info("向MQ发送视频处理消息成功!");
